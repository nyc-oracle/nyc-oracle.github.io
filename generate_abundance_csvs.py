...
def build_daily_frame(raw, date_col, count_col, count_rename="count"):
    """
    ...
    df = df.set_index(date_col).reindex(full_dates, fill_value=0).reset_index()
...
def main():
    ...
    datasets = [
        # (output stem, fetch_fn, count_col in API response, count_col in CSV)
        ("311_noise_street_2024_abundance",
         lambda: fetch_311_daily("Noise - Street/Sidewalk"), "count", "count"),
        ("311_heat_hotwater_2024_abundance",
         lambda: fetch_311_daily("HEAT/HOT WATER"), "count", "count"),
        ("311_unsanitary_2024_abundance",
         lambda: fetch_311_daily("UNSANITARY CONDITION"), "count", "count"),
        ("311_street_condition_2024_abundance",
         lambda: fetch_311_daily("Street Condition"), "count", "count"),
        ("311_dhs_2024_abundance",
         lambda: fetch_311_daily(agency="DHS"), "count", "count"),
        ("dog_bites_2024_abundance",
         fetch_dog_bites_daily, "bite_count", "bite_count"),
        ("ferry_ridership_2024_abundance",
         fetch_ferry_daily, "count", "count"),
        ("arrests_2024_abundance",
         fetch_arrests_daily, "count", "count"),
    ]

    failed = []
    for stem, fetch_fn, count_col, count_rename in datasets:
        print(f"\n[{stem}]")
        raw = fetch_fn()
        df = build_daily_frame(raw, "date", count_col, count_rename)
        if df is not None:
            save_csv(df, stem)
        else:
